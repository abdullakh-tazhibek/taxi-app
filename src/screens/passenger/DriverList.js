...
import {
  ...
  openModal,
  closeModal,
} from "../../../redux/passenger/driverList";

export default function DriverList() {
  const dispatch = useDispatch();
  const { fullData, isLoaded, driverList, modalVisible, selectedItem } =
  ...
  const updateSearchValues = React.useCallback(
    debounce((location1, location2) => {
      if (!location1 && !location2) {
        dispatch(setDriverList(fullData));
      } else {
        ...
        dispatch(setDriverList(filteredData));
      }
    }, 150),
    [dispatch, fullData]
  );
  ...
  const [opnd, setOpnd] = React.useState(false);
  ...
  return (
    <View style={{ flex: 1, marginTop: 24 }}>
      ...
      >
        ...
        <Pressable onPress={() => setOpnd(true)}>
          <Image
      ...
      {isLoaded === true ? (
        Array.isArray(driverList) && driverList.length > 0 ? (
          <View style={{ flex: 1 }}>
            <FlatList
              data={driverList}
      ...
      {opnd ? (
        ...
        >
          <Card>
            <Text style={{ fontSize: 16, color: "#696682", marginBottom: 12 }}>
              Павлодардан көлік таппасаңыз, Қарағанды, Астанадан іздесеңіз
              болады.
            </Text>
            <Button
              onPress={() => setOpnd(false)}
              style={{
                fontSize: 12,
                marginTop: 12,
